...
import { Education, education as initialData } from '../education';

const TABLE_NAME = 'education';

export const educationService = {
    async getAll(): Promise<(Education & { id: string })[]> {
        const { data, error } = await supabase.from(TABLE_NAME).select('*');
        if (error) { console.error('Error:', error); return []; }
        return data as (Education & { id: string })[];
    },
    async create(item: Education) {
        const { data, error } = await supabase.from(TABLE_NAME).insert(item).select().single();
        if (error) throw error;
        return data;
    },
    async update(id: string, item: Partial<Education>) {
