    ...
    <div className="flex flex-col h-full bg-[#ECE9D8] font-tahoma text-[11px] select-none">
      <Tabs defaultValue="Settings" className="flex flex-col h-full">
          ...
          <TabsList className="bg-transparent p-0 h-auto gap-1 justify-start w-full rounded-none">
            {["Themes", "Desktop", "Screen Saver", "Appearance", "Settings"].map((tab) => (
              <TabsTrigger
                ...
                value={tab}
                className={cn(
                  "px-3 py-1 rounded-t border-t border-l border-r border-gray-400 relative -bottom-[1px] rounded-b-none border-b-0 h-auto shadow-none",
        ...
        <div className="flex-1 p-4 bg-white border border-gray-400 m-2 mt-0 flex flex-col">
          ...
          {/* Other tabs content placeholders if needed, but for now we only have Settings content implemented */}
          {["Themes", "Desktop", "Screen Saver", "Appearance"].map((tab) => (
             <TabsContent key={tab} value={tab} className="mt-0 flex flex-col h-full items-center justify-center text-gray-500">
