...
export function ProjectsViewer() {
  const [projectsData, setProjectsData] = useState<Project[]>(staticProjects);
  ...
  return (
    <div className="flex h-full bg-white font-tahoma text-xs">
      ...
      <div className="w-48 bg-[#7BA2E7] p-3 flex flex-col gap-3 overflow-y-auto shrink-0" style={{ background: "linear-gradient(to bottom, #7BA2E7 0%, #6375D6 100%)" }}>
        ...
        {/* Details Box */}
        <Card className="bg-white rounded-t-sm overflow-hidden border-none shadow-none rounded-none">
          ...
          <CardContent className="p-3 bg-[#D6DFF7] min-h-[100px]">
            {selectedProject ? (
              <div className="flex flex-col gap-2">
                <span className="font-bold text-black">{projectsData.find(p => p.id === selectedProject)?.title}</span>
                <span className="text-gray-600">{projectsData.find(p => p.id === selectedProject)?.type}</span>
                <div className="h-[1px] bg-[#A0A0A0]" />
                <span className="text-black">{projectsData.find(p => p.id === selectedProject)?.description}</span>
                <hr />
                <span className="text-black">{projectsData.find(p => p.id === selectedProject)?.tech}</span>
                {projectsData.find(p => p.id === selectedProject)?.link && (
                  <>
                    <div className="h-[1px] bg-[#A0A0A0]" />
                    <a
                      href={projectsData.find(p => p.id === selectedProject)?.link}
                      target="_blank"
      ...
      <div className="flex-1 flex flex-col bg-white min-h-0">
        ...
        <div className="border-b border-[#D6D3CE] p-1 flex items-center gap-2 bg-[#ECE9D8]">
          ...
          <div className="flex-1 bg-white border border-[#7F9DB9] px-1 py-0.5 flex items-center gap-1 h-6">
            <img src="/folder.ico" className="w-4 h-4" />
            <Input
              readOnly
              value="C:\Desktop\My Projects"
              className="border-none h-auto p-0 shadow-none focus-visible:ring-0 rounded-none text-xs font-tahoma"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col relative">
          <ScrollArea className="h-full w-full">
            <div className="p-4 pb-8 flex flex-col min-h-full">
                      ...
                      <div className="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-4 content-start px-4 animate-in slide-in-from-top-2 duration-200">
                        {projectsData.filter(p => p.category === category).map((project) => (
                          <Card
                            ...
                            className={`
                              group flex flex-col items-center gap-1 p-2 border border-transparent rounded hover:bg-[#E8F0FA] hover:border-[#B2C7E6] cursor-pointer shadow-none bg-transparent
              ...
              {selectedProject && (
                <div className="flex flex-col gap-1 animate-in fade-in duration-300 mt-auto pt-4 border-t border-[#D6D3CE] max-w-5xl mx-auto w-full">
                  <div className="flex items-center justify-between bg-gradient-to-r from-[#0054E3] to-[#2F89F5] px-2 py-1 rounded-t-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-white truncate">
                        {projectsData.find(p => p.id === selectedProject)?.link ? "Live Preview" : "Preview Unavailable"} - {projectsData.find(p => p.id === selectedProject)?.title}
                      </span>
                    </div>
                    {projectsData.find(p => p.id === selectedProject)?.link && (
                      <Button
                        variant="link"
                        className="text-white hover:underline text-[10px] whitespace-nowrap ml-2 h-auto p-0 hover:text-white"
                        asChild
                      >
                        <a
                          href={projectsData.find(p => p.id === selectedProject)?.link}
                  ...
                  <div className="border-2 border-[#0054E3] bg-white w-full shadow-md relative overflow-hidden">
                    {projectsData.find(p => p.id === selectedProject)?.link ? (
                      <iframe
                        src={projectsData.find(p => p.id === selectedProject)?.link}
        ...
        {/* Status Bar */}
        <div className="border-t border-[#D6D3CE] bg-[#ECE9D8] px-2 py-0.5 flex gap-4 text-black shrink-0">
          <span>{projectsData.length} items</span>
        </div>
      </div>
    </div>
  );
